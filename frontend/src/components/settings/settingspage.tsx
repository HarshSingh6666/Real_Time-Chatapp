...
interface SettingsPageProps {
  ...
  onBack: () => void; // Main chat pe wapas jane ke liye function pass karo
}

type SettingsTab = 'profile' | 'notifications' | 'privacy' | 'appearance' | 'help';
type ThemeType = 'dark' | 'light' | 'system';

export const SettingsPage = ({ user, onUserUpdate, onBack }: SettingsPageProps) => {
  ...
  return (
    ...
    <div className="w-full h-[100dvh] bg-background flex flex-row overflow-hidden relative">
      ...
      {imageSrc && (
          <div className="absolute inset-0 z-50 bg-background flex flex-col">
            ...
            <div className="p-4 md:p-6 bg-background border-t space-y-4">
                ...
                <div className="flex gap-3 justify-center md:justify-end">
                  <button onClick={() => setImageSrc(null)} className="flex-1 md:flex-none px-6 py-3 md:py-2 rounded-xl font-bold text-muted-foreground hover:bg-secondary transition-colors">Cancel</button>
                  <button onClick={handleUploadCroppedImage} disabled={imageLoading} className="flex-1 md:flex-none px-6 py-3 md:py-2 rounded-xl font-bold bg-primary text-primary-foreground hover:opacity-90 flex items-center justify-center gap-2 transition-opacity">
      ...
      {showMobileMenu && (
        <div 
          className="absolute inset-0 bg-black/60 z-40 lg:hidden animate-in fade-in" 
      ...
      <nav 
        className={cn(
          "absolute lg:relative z-50 h-full overflow-y-auto bg-background lg:bg-secondary/10 p-4 sm:p-6 flex-shrink-0 flex flex-col transition-transform duration-300 border-r border-border shadow-2xl lg:shadow-none",
          "w-[280px] lg:w-72", 
          showMobileMenu ? "translate-x-0" : "-translate-x-full lg:translate-x-0" // Drawer effect on Mobile/Tablet
        )}
      >
        <div className="space-y-6 md:space-y-8 flex-1">
          <div className="flex items-center justify-between lg:justify-start pt-2 lg:pt-0 mb-4 lg:mb-0">
              ...
              <button onClick={() => setShowMobileMenu(false)} className="lg:hidden p-2 bg-secondary rounded-full hover:bg-secondary/80 transition-colors">
                  <X size={20} />
              </button>
          </div>
          <div className="space-y-2 mt-4">
            {tabs.map((tab) => (
              <button
                ...
                className={cn(
                  'w-full flex items-center gap-4 px-4 py-3.5 rounded-2xl text-[15px] font-bold transition-all duration-200',
                  activeTab === tab.id
                    ? 'bg-primary text-primary-foreground shadow-xl shadow-primary/20 scale-[1.02]' 
                    : 'text-muted-foreground hover:bg-secondary/80 hover:text-foreground'
                )}
              >
                <tab.icon size={22} strokeWidth={2.5} className="flex-shrink-0" />
                {tab.label}
                <ChevronRight size={18} className="ml-auto opacity-50 lg:hidden flex-shrink-0" />
              </button>
            ))}
          </div>
        </div>
        <button onClick={handleLogout} className="flex items-center justify-center lg:justify-start gap-4 px-4 py-4 mt-8 lg:mt-0 rounded-2xl text-[15px] font-bold text-destructive hover:bg-destructive/10 transition-all border border-transparent hover:border-destructive/20">
          <LogOut size={20} className="flex-shrink-0" /> Log Out
        </button>
      </nav>
      ...
      <div className="h-full relative min-w-0 flex-1 bg-card/5 flex flex-col w-full">
        {/* Header */}
        <div className="p-4 md:p-6 border-b border-border flex items-center justify-between bg-background/80 backdrop-blur-xl sticky top-0 z-10 flex-shrink-0">
          <div className="flex items-center gap-3 text-muted-foreground">
            {/* 🔥 Hamburger Button (Hidden on Large screens) */}
            <button 
              onClick={() => setShowMobileMenu(true)} 
              className="lg:hidden p-1.5 -ml-1.5 hover:bg-secondary rounded-lg transition-colors text-foreground"
            >
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-lg md:text-base font-bold text-foreground capitalize">{activeTab}</span>
            </div>
          </div>

          {/* Go Back to Chat button */}
          <button onClick={onBack} className="flex items-center gap-2 px-4 py-2 bg-secondary/50 hover:bg-secondary rounded-xl font-bold transition-all text-sm text-foreground">
            <ArrowLeft size={16} /> <span className="hidden sm:block">Back to Chat</span>
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 md:p-8 custom-scrollbar">
          {/* ---------------- PROFILE TAB ---------------- */}
          {activeTab === 'profile' && (
            <div className="space-y-6 lg:space-y-8 animate-in fade-in duration-500 max-w-3xl mx-auto pb-10">
              <div className="flex flex-col sm:flex-row items-center gap-5 sm:gap-6 p-5 sm:p-6 rounded-[2rem] bg-gradient-to-br from-primary/10 via-transparent to-transparent border border-primary/10">
                <div className="relative group flex-shrink-0">
                  <div className="h-28 w-28 md:h-32 md:w-32 rounded-full p-1 bg-gradient-to-tr from-primary to-purple-500 shadow-xl relative overflow-hidden">
                      ...
                      <div className="absolute inset-0 bg-black/40 opacity-100 lg:opacity-0 lg:group-hover:opacity-100 transition-opacity flex flex-col lg:flex-row items-center justify-center gap-2 rounded-full z-10">
                ...
                <div className="text-center sm:text-left space-y-1 w-full min-w-0">
                  <h4 className="text-2xl md:text-3xl font-black text-foreground truncate">{user.name}</h4>
                  <p className="text-primary font-bold opacity-80 truncate text-lg">@{user.username}</p>
                  <div className="flex gap-2 justify-center sm:justify-start mt-2">
                      <span className="px-3 py-1 rounded-full bg-green-500/10 text-green-500 text-xs font-bold border border-green-500/20">VERIFIED</span>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                {isEditing && (
                  <div className="pt-4 flex justify-end">
                    <button onClick={handleSaveProfile} disabled={loading} className="w-full sm:w-auto flex items-center justify-center gap-2 bg-primary text-primary-foreground px-8 py-3.5 rounded-xl font-bold hover:opacity-90 transition-all shadow-lg shadow-primary/20 disabled:opacity-50">
          ...
          {activeTab === 'notifications' && (
            <div className="flex flex-col items-center justify-center h-full min-h-[50vh] text-center space-y-4 animate-in fade-in duration-500">
                <div className="h-20 w-20 md:h-24 md:w-24 bg-primary/10 rounded-full flex items-center justify-center text-primary mb-2 md:mb-4 relative">
                  <BellOff size={32} className="md:w-10 md:h-10" />
                  <span className="absolute top-0 right-0 h-3 w-3 md:h-4 md:w-4 bg-yellow-500 rounded-full animate-ping" />
                </div>
                <h3 className="text-xl md:text-2xl font-black text-foreground">Work in Progress</h3>
                <p className="text-sm md:text-base text-muted-foreground max-w-sm leading-relaxed px-4">Notifications are currently under development. <br className="hidden md:block" /><span className="font-bold text-primary">This feature will be fully functional after the next update.</span></p>
            </div>
          )}
          ...
          {activeTab === 'privacy' && (
            <div className="space-y-6 animate-in fade-in duration-500 max-w-3xl mx-auto pb-10">
              <div className="space-y-1 md:space-y-2"><h3 className="text-xl md:text-2xl font-black">Privacy & Terms</h3><p className="text-sm text-muted-foreground">Please read our terms and conditions carefully.</p></div>
              <div className="p-5 md:p-6 rounded-3xl bg-secondary/20 border border-border/50 text-sm space-y-6 md:max-h-[500px] overflow-y-auto custom-scrollbar">
          ...
          {activeTab === 'appearance' && (
            <div className="space-y-6 md:space-y-8 animate-in fade-in duration-500 max-w-3xl mx-auto pb-10">
              <div className="space-y-1 md:space-y-2"><h3 className="text-xl font-black text-foreground">Visual Style</h3><p className="text-sm text-muted-foreground">Customize how OpenTalks looks on your device.</p></div>
          ...
          {activeTab === 'help' && (
            <div className="space-y-8 animate-in fade-in duration-500 max-w-3xl mx-auto pb-10">
              <div className="text-center space-y-2 pt-4 md:pt-0"><h3 className="text-2xl md:text-3xl font-black bg-gradient-to-br from-foreground to-muted-foreground bg-clip-text text-transparent">We're here to help</h3></div>
              <div className="grid gap-4">
                  <a href="mailto:OpenTalks@gmail.com" className="group flex flex-col sm:flex-row items-center sm:items-start text-center sm:text-left gap-4 md:gap-5 p-5 rounded-[2rem] bg-gradient-to-br from-primary/10 via-card to-card border border-primary/20 hover:scale-[1.02] transition-all duration-300">
...
const InputGroup = ({ icon, label, value, onChange, disabled, type = "text", placeholder, className }: any) => (
  <div className={cn("space-y-1.5 md:space-y-2 w-full", className)}>
    <label className="text-[10px] md:text-xs font-bold uppercase tracking-wider text-muted-foreground ml-1">{label}</label>
    <div className={cn("flex items-center gap-3 p-3.5 md:p-4 rounded-2xl border transition-all duration-200", disabled ? "bg-secondary/30 border-transparent" : "bg-background border-primary/50 ring-2 ring-primary/10")}>
      <span className={cn(disabled ? "text-muted-foreground" : "text-primary flex-shrink-0")}>{icon}</span>
      <input type={type} value={value} onChange={onChange} disabled={disabled} placeholder={placeholder} className="flex-1 w-full bg-transparent outline-none font-bold text-sm md:text-base text-foreground placeholder:font-normal placeholder:text-muted-foreground/50 truncate" />
    </div>
  </div>
);